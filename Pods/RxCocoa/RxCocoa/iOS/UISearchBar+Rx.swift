...
extension Reactive where Base: UISearchBar {
    ...
    public var value: ControlProperty<String?> {
        let source: Observable<String?> = Observable.deferred { [weak citySearchBar = self.base as UISearchBar] () -> Observable<String?> in
            let text = citySearchBar?.text

            let textDidChange = (citySearchBar?.rx.delegate.methodInvoked(#selector(UISearchBarDelegate.searchBar(_:textDidChange:))) ?? Observable.empty())
            let didEndEditing = (citySearchBar?.rx.delegate.methodInvoked(#selector(UISearchBarDelegate.searchBarTextDidEndEditing(_:))) ?? Observable.empty())
            
            return Observable.merge(textDidChange, didEndEditing)
                    .map { _ in citySearchBar?.text ?? "" }
                    .startWith(text)
        }
        
        let bindingObserver = Binder(self.base) { (citySearchBar, text: String?) in
    ...
    public var selectedScopeButtonIndex: ControlProperty<Int> {
        ...
        let bindingObserver = Binder(self.base) { (citySearchBar, index: Int) in
            citySearchBar.selectedScopeButtonIndex = index
        }
